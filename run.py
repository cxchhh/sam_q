...
from utils.generate import infer, change_sam_ver
...
with blocks as demo:
    with gradio.Row(equal_height=True):
        with gradio.Column(scale=1,min_width=120):
            ...
            model_type.change(change_sam_ver, model_type,model_type,js="(x)=>{globalThis.model_type=x; return x;}")
        ...
        with gradio.Column(scale=5):
            with gradio.Row():
                outputImg = gradio.Image(label="origin model output",type="numpy")
                results = gradio.Textbox(interactive=False)
            with gradio.Row():
                outputImg_q = gradio.Image(label="quantized model output",type="numpy")
                results_q = gradio.Textbox(interactive=False)
